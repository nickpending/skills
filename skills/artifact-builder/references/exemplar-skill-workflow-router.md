# Exemplar: Workflow Router Skill

**Pattern:** Workflow Router
**Source:** homenet-discovery/SKILL.md
**Lines:** 132
**Use when:** Conditional routing to different workflows based on state or user intent
...
- Overview with discovery methods and outputs
- Workflow checkbox (copy to track progress)
- Execute ALL steps in sequence (explicit routing)
- Available Scripts (inventory)
- Supporting References (pointers)
- When to Use (triggers and use cases)
- Data Locations (config and output paths)
...
1. **Workflow checkbox:** Copyable progress tracker
2. **Execute ALL steps in sequence:** Explicit instruction header
3. **Conditional routing:** CHECK state → IF/ELSE → load appropriate workflow
4. **Script inventory:** Grouped by function (Discovery, Processing, Output)
5. **Reference pointers:** Links to bundled workflows and references
6. **Triggers section:** Both natural language and use cases
7. **Data locations:** Configuration and output paths documented
8. **Reconfiguration:** How to reset state
