# Exemplar: Multi-Command CLI Skill

**Pattern:** Multi-Command CLI
**Source:** lore/SKILL.md
**Lines:** 154
**Use when:** Wrapping a CLI with multiple subcommands and complex intent mapping
...
- Overview with IMPORTANT warning
- Commands section (grouped by function)
- Intent Mapping (natural language → command)
- Common Cypher (domain-specific examples)
- Schema (quick reference)
- Execution (parse → construct → execute pattern)
...
1. **Intent Mapping section:** Natural language phrases → exact commands
2. **Grouped commands:** Organized by function (List, Search, Graph, Index, Personal)
3. **Domain-specific content:** Cypher examples for graph queries
4. **Schema reference:** Quick lookup for node/edge types
5. **Execution pattern:** Clear parse → construct → execute flow
6. **Reindex reminders:** After mutations, remind to rebuild indices
